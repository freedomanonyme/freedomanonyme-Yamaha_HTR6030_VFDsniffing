...
# AUTO_LOAD est plus robuste pour les composants personnalisés
...
CONFIG_SCHEMA = cv.Schema({
    ...
    cv.Optional(CONF_SOURCE_SENSOR): cv.Any(text_sensor.text_sensor_schema(), cv.use_id(text_sensor.TextSensor)),
    cv.Optional(CONF_VOLUME_SENSOR): cv.Any(text_sensor.text_sensor_schema(), cv.use_id(text_sensor.TextSensor)),
    cv.Optional(CONF_MUTE_SENSOR): cv.Any(binary_sensor.binary_sensor_schema(), cv.use_id(binary_sensor.BinarySensor)),
...
async def to_code(config):
    ...
    if CONF_SOURCE_SENSOR in config:
        conf = config[CONF_SOURCE_SENSOR]
    ...
    if CONF_VOLUME_SENSOR in config:
        conf = config[CONF_VOLUME_SENSOR]
        if isinstance(conf, dict):
            sens = await text_sensor.new_text_sensor(conf)
        else:
            sens = await cg.get_variable(conf)
